...
const mwSignedIndex log4[4]   = {0,0,1,2}; /* stores i at address \alpha^i */

const mwSignedIndex log8[8]   = {0,0,1,3,2,6,4,5};

const mwSignedIndex log16[16] = {0,0,1,4,2,8,5,10,3,14,9,7,6,13,11,12};

const mwSignedIndex log32[32] = {0,0,1,18,2,5,19,11,3,29,6,27,20,8,12,23,4,
10,30,17,7,22,28,26,21,25,9,16,13,14,24,15};

const mwSignedIndex log64[64] = {0,0,1,6,2,12,7,26,3,32,13,35,8,48,27,18,4,24,
...
const mwSignedIndex log128[128] = {0,0,1,31,2,62,32,103,3,7,63,15,33,84,104,
...
const mwSignedIndex log256[256] = {0,0,1,25,2,50,26,198,3,223,51,238,27,104,199,75,4,100,
...
const mwSignedIndex exp4[3]   = {1,2,3}; /* stores \alpha^i at address i */

const mwSignedIndex exp8[7]   = {1,2,4,3,6,7,5};

const mwSignedIndex exp16[15] = {1,2,4,8,3,6,12,11,5,10,7,14,15,13,9};

const mwSignedIndex exp32[31] = {1,2,4,8,16,5,10,20,13,26,17,7,14,28,29,31,
27,19,3,6,12,24,21,15,30,25,23,11,22,9,18};

const mwSignedIndex exp64[63] = {1,2,4,8,16,32,3,6,12,24,48,35,5,10,20,40,19,
...
const mwSignedIndex exp128[127] = {1,2,4,8,16,32,64,9,18,36,72,25,50,100,65,11,
...
const mwSignedIndex exp256[255] = {1,2,4,8,16,32,64,128,29,58,116,232,205,135,19,38,76,
...
mwSignedIndex GFq_m(mwSignedIndex a, mwSignedIndex b, mwSignedIndex q)
{
    if ( a == 0 || b == 0 )  return 0 ;
    if ( a == 1 )  return b ;
    if ( b == 1 )  return a ;
...
mwSignedIndex GFq_inv(mwSignedIndex a, mwSignedIndex q)
{
...
mwSignedIndex GFq_a(mwSignedIndex a, mwSignedIndex b)
{
    return a^b;
}
...
void mexFunction(
mwSignedIndex nlhs,       mxArray *plhs[],
mwSignedIndex nrhs, const mxArray *prhs[]
)

{
    unsigned char **HH, **GG;
    mwSignedIndex ii, jj, *ir, *jc, rdep, tmp, d, q, scale;
    double *sr1, *sr2, *g;
    mwSignedIndex N,M,K,i,j,k,kk,nz,*irs1,*jcs1, *irs2, *jcs2;
...
/* get the field base */
    ...
    q = (mwSignedIndex)mxGetScalar(Q_IN);
...
/* invert HH matrix here */
    ...
    ir = (mwSignedIndex *)mxMalloc(M*sizeof(mwSignedIndex));
    jc = (mwSignedIndex *)mxMalloc(N*sizeof(mwSignedIndex));
...
/* NOTE, it is a very inefficient way to store G. Change to taste!*/
    ...
    /* Assign pomwSignedIndexers to the output matrix */
    g = mxGetPr(G_OUT);
